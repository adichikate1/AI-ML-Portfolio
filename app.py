...
# test_pred = house_model.predict(X_house_test)
...
#         price = float(house_model.predict(input_data)[0])
...
model_path = "static/models/Customer_churn.keras"

print(zipfile.is_zipfile(model_path))

Customer_model = load_model("static/models/Customer_churn.h5")
label_encoders = joblib.load("static/models/gender_encoder.pkl")
standard_encoders = joblib.load("static/models/standard_encoder.pkl")


@app.route("/Churn_ML_Predictor", methods=["GET", "POST"])
def churn_ml_predictor():
    prediction = None
    probability = None
    ...
    if request.method == "POST":
        ...
        # df_customer["Gender"] = label_encoders.transform(df_customer[["Gender"]])
        df_customer = standard_encoders.transform(df_customer)

        prediction = Customer_model.predict(df_customer)
    ...
    return render_template("Customer Churn.html", exited=prediction, probability=probability)
...
CNN_Model = load_model("static/models/best_model_epoch80.h5")

@app.route("/cat_vs_dog", methods=['GET','POST'])
def cat_vs_dog():
    ...
    if request.method == "POST":
        ...
        img = Image.open(photo.stream)      # FileStorage -> PIL Image
        img = img.convert("RGB")            # RGB ensure karo
        ...
        if probability > 0.5:
            ...
            dog_probability = round(confidence, 2)
            cat_probability = round(100 - confidence, 2)
        else:
            predicted_class = "cat"
            confidence = round((1 - probability) * 100, 2)
            cat_probability = round(confidence, 2)
            dog_probability = round(100 - confidence, 2)
        
    return render_template("Cat vs Dog.html", predicted_class=predicted_class, confidence=confidence, cat_probability=cat_probability, dog_probability=dog_probability)
